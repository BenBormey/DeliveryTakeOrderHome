        ...
        private void TimerCustomerLoading_Tick_1(object sender, EventArgs e)
        {
            ...
            if (this.RdbManually.Checked)
            {
                if (this.ProgramName.Equals("Sale_Team"))
                {
                    ...
                    --AND [CusNum] IN (SELECT [CusNum] FROM [Stock].[dbo].[TPRDeliveryTakeOrder_SetBillTo] GROUP BY [CusNum])
                    ORDER BY [CusName];
                ";
                }
                else
...
DROP TABLE #lst_items;
        ";
            ...
            this.query = string.Format(this.query, DatabaseName, Initialized.vIsNestleOnly && !this.chkallitems.Checked ? "" : "--", AppSetting.SaleManagerID);
        ...
        private void TimerLoading_Tick(object sender, EventArgs e)
        {
            ...
            InvNo += 1;
            TxtInvoiceNo.Text = InvNo.ToString();
...
";
        ...
        private void CmbBillTo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            App.ClearController(TxtIdDelto, CmbDelto, TxtZone, TxtKhmerName, TxtNote, txtpromotionid, CmbProducts, TxtQtyPerCase, TxtStock, TxtBarcodeFree, TxtPcsOrder, TxtPackOrder, TxtCTNOrder, TxtTotalPcsOrder, TxtUnitPrice, TxtPcsFree, TxtItemDiscount, TxtWSPrice, TxtTotalAmount, TxtRequiredDate);
            this.chknone.Checked = false;
...
WITH lDocumentARIn AS (
    SELECT [ll].[allowedday],
...
";
            ...
            if (this.lstsd_ != null)
            {
                ...
                {
                    string sdId_ = (from x in this.lstsd_ where x.cusNum.Trim().Equals(CusNum) select x.id).FirstOrDefault();
                    this.loginToken();
                    ...
                    if (this.lstcheckallowance_ != null)
                    {
                        if (this.lstcheckallowance_.invoices > 0)
                        {
                            ...
                            MessageBox.Show("Please ask Sub Distributor to update arrival, thank you!", "Not Allow To Continue", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return;
                        }
                    }
                }
...
";
            ...
            if (this.CheckBankGaranteeCustomer(CusNum))
                return;
            // Check Credit Amount
            if (!this.CheckCreditAmountCustomer(CusNum))
                return;
...
FROM lCustomerList lc
    ...
    ";
            ...
            if (lists != null)
            {
                ...
                {
                    this.vServiceCost = Convert.ToDouble(DBNull.Value.Equals(lists.Rows[0]["ServiceCost"]) ? 0 : lists.Rows[0]["ServiceCost"]);
                    this.CusItemDis = Convert.ToSingle(DBNull.Value.Equals(lists.Rows[0]["Discount"]) ? 0 : lists.Rows[0]["Discount"]);
                    this.CusInvoiceDis = Convert.ToSingle(DBNull.Value.Equals(lists.Rows[0]["InvoiceDiscount"]) ? 0 : lists.Rows[0]["InvoiceDiscount"]);
                    this.CusVAT = Convert.ToString(DBNull.Value.Equals(lists.Rows[0]["CusVat"]) ? "" : lists.Rows[0]["CusVat"]).Trim();
                    this.CusServiceRebate = Convert.ToSingle(DBNull.Value.Equals(lists.Rows[0]["ServiceRebate"]) ? 0 : lists.Rows[0]["ServiceRebate"]);
                    this.Terms = Convert.ToInt32(DBNull.Value.Equals(lists.Rows[0]["Terms"]) ? 0 : lists.Rows[0]["Terms"]);
                    this.CreditLimit = Convert.ToDouble(DBNull.Value.Equals(lists.Rows[0]["CreditLimit"]) ? 0 : lists.Rows[0]["CreditLimit"]);
                    this.MaxMonthAllow = Convert.ToInt32(DBNull.Value.Equals(lists.Rows[0]["MaxMonthAllow"]) ? 0 : lists.Rows[0]["MaxMonthAllow"]);
                    this.CreditLimitAllow = Convert.ToDouble(DBNull.Value.Equals(lists.Rows[0]["CreditLimitAllow"]) ? 0 : lists.Rows[0]["CreditLimitAllow"]);
                    this.vCity = Convert.ToString(DBNull.Value.Equals(lists.Rows[0]["City"]) ? "" : lists.Rows[0]["City"]).Trim();
                    this.vIssueUnitPrice = Convert.ToBoolean(DBNull.Value.Equals(lists.Rows[0]["IssueUnitPrice"]) ? 0 : lists.Rows[0]["IssueUnitPrice"]);
                    this.vAdditionalCost = Convert.ToBoolean(DBNull.Value.Equals(lists.Rows[0]["AdditionalCost"]) ? 0 : lists.Rows[0]["AdditionalCost"]);
                    this.vIssueUnitPrice = Convert.ToBoolean(DBNull.Value.Equals(lists.Rows[0]["IssueUnitPrice"]) ? 0 : lists.Rows[0]["IssueUnitPrice"]);
...
";
            ...
            var CreditAmountList = Data.Selects(query, Initialized.GetConnectionType(Data, App));
            if (CreditAmountList != null)
            {
                if (CreditAmountList.Rows.Count > 0)
                {
                    var firstRow = CreditAmountList.Rows.Cast<DataRow>().FirstOrDefault();
                    InvNumber = firstRow["invNum"].ToString();
                    ShipDate = Convert.ToDateTime(firstRow["invDate"]);
...
";
            ...
            if (lists != null)
            {
                if (lists.Rows.Count > 0)
                {
                    var row = lists.Rows[0];
                    int qtyInvSetted = Convert.ToInt32(DBNull.Value.Equals(row["qtyInvSetted"]) ? "0" : row["qtyInvSetted"]);
                    termOver = DBNull.Value.Equals(row["termOver"]) ? 0 : Convert.ToInt32(lists.Rows.Cast<DataRow>().FirstOrDefault()["termOver"]);
                    termOverCredit = DBNull.Value.Equals(row["termOverCredit"]) ? 0 : Convert.ToInt32(lists.Rows.Cast<DataRow>().FirstOrDefault()["termOverCredit"]);
                    CreditAmount += DBNull.Value.Equals(row["creditOver"]) ? 0 : lists.Rows.Cast<DataRow>().Sum(lu => Convert.ToDouble(lu["creditOver"]));
                    if (!DBNull.Value.Equals(row["termSetted"]) && qtyInvSetted > 0)
                    {
                        Terms = lists.Rows.Cast<DataRow>().Sum(lu => Convert.ToInt32(lu["termSetted"]));
                    }
                    if (!DBNull.Value.Equals(row["creditSetted"]) && qtyInvSetted > 0)
                    {
                        CreditLimit = lists.Rows.Cast<DataRow>().Sum(lu => Convert.ToDouble(lu["creditSetted"]));
                    }
                }
            }
            CreditAmount += Amount;
            ...
            else if (termOver >= Terms)
            {
                ...
                if (ofrm_.ShowDialog() == DialogResult.Yes)
                {
                    if (CreditAmount >= CreditLimitAllow)
                    {
                        ...
                        var lG1 = new FrmPasswordContinues();
                        lG1.R_PasswordPermission = "'Managing Director','IT Manager'";
                        lG1.ShowDialog();
                        if (Initialized.R_CorrectPassword)
                        {
                            App.SetEnableController(true, BtnAdd, CmbProducts, CmbDelto, CmbCategory);
                            IsAllow = true;
                        }
        ...
        {
            if (RdbOnlinePO.Checked) { this.sFirstLoading = true; this.lblMsg.Text = this.RdbOnlinePO.Text.Trim().ToUpper(); this.TimerCustomerLoading.Enabled = true; }
        }
...
";
        ...
        private void ClearProductItems()
        {
            ...
            this.PicProducts.Image = null;
            this.App.ClearController(TxtQtyPerCase, TxtStock, TxtWSPrice, TxtBarcodeFree, TxtPcsFree, TxtItemDiscount, TxtPcsOrder, TxtPackOrder, TxtCTNOrder, TxtTotalPcsOrder, TxtUnitPrice, TxtPackPrice, TxtTotalAmount, TxtRemark);
...
";
            ...
            if (dtDC.Rows.Count > 0)
            {
                ...
                dcReport = new MailDCSKUReport();
                DataRow dr = dtDC.Rows[0];
                //dcReport.paramDear.Value = $"Dear {CmbBillTo.Text},";
                //dcReport.paramPO.Value = TxtPONo.Text.Replace("'", "`").Trim();
                //dcReport.paramBarcode.Value = dr["ProNumY"].ToString();
                //dcReport.paramName.Value = dr["ProName"].ToString();
                //dcReport.paramSize.Value = dr["ProPacksize"].ToString();
...
";
            ...
            check_pending_stock.Enabled = true;
...
";
            ...
            this.lists = Data.Selects(this.query, Initialized.GetConnectionType(Data, App));
            List<cls_check_stock> lst_check_stock_ = this.db.GetDataTableToObject<cls_check_stock>(this.lists);
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            else if (TxtRequiredDate.Text.Trim() == "")
            {
    ...
    ";
                ...
                if (lists != null)
                {
                    if (lists.Rows.Count > 0)
                    {
                        ...
                        if (vDeliveryLogistic == 0) MessageBox.Show("Please check Additional Cost of product for this customer first!" + Environment.NewLine + "Please contact to Administrator.", "Not Allow To Continue", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.Cursor = Cursors.Default;
                        return;
                    }
                    else
...
DROP TABLE #lst_takeorder_{4};
        ...
        WHERE [TakeOrderNumber] = @TakeOrderNumber;
    ";
                                    sql_ = string.Format(sql_, DatabaseName, InvNo, Initialized.R_MessageAlert.Trim());
                                    RCom.CommandText = sql_;
                                    RCom.ExecuteNonQuery();
                                }
    ...
    ";
                                    sql_ = string.Format(sql_, DatabaseName, InvNo, CmbBillTo.SelectedValue, Initialized.R_DocumentNumber.Trim(), Initialized.R_LineCode.Trim(), Initialized.R_DeptCode.Trim());
                                    RCom.CommandText = sql_;
                                    RCom.ExecuteNonQuery();
                                }
...
DELETE FROM [DBUNTWHOLESALECOLTD].[dbo].[TblDeliveryTakeOrders] WHERE ([TakeOrderNumber] = @takeOrderNumber);";
        ...
        private void TxtSearchStoreCode_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            if (e.KeyCode == Keys.Return)
            {
                if (string.IsNullOrWhiteSpace(TxtSearchStoreCode.Text.Trim())) return;
                e.IsInputKey = true;
                string StoreCode = string.IsNullOrWhiteSpace(TxtSearchStoreCode.Text.Trim()) ? "" : TxtSearchStoreCode.Text.Trim();
                string LinkBarcode = "";
                string query = @"
        DECLARE @CusNum AS NVARCHAR(8) = N'{1}';
    ...
    ";
                ...
                if (lists != null)
                {
                    if (lists.Rows.Count > 0)
                    {
                        LinkBarcode = string.IsNullOrWhiteSpace(Convert.IsDBNull(lists.Rows[0]["Barcode"]) ? string.Empty : lists.Rows[0]["Barcode"].ToString().Trim())
      ? string.Empty
      : lists.Rows[0]["Barcode"].ToString().Trim();
        ...
        private void DeactivatedProduct()
        {
            ...
            //    MailDCSKUReport dcReport = new MailDCSKUReport();
            //    DataRow dr = dtDC.Rows[0];
            //    dcReport.paramDear.Value = string.Format("Dear {0},", CmbBillTo.Text);
            //    dcReport.paramPO.Value = TxtPONo.Text.Replace("'", "`").Trim();
            //    dcReport.paramBarcode.Value = dr["ProNumY"];
            //    dcReport.paramName.Value = dr["ProName"];
            //    dcReport.paramSize.Value = dr["ProPacksize"];
            ...
            //        {
            //            string email = QueryCCEmail.Rows[0][0].ToString();
